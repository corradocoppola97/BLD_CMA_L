...
def train_model(sm_root: str,
    ...
    if ablate is not None:
        ...
        for k in range(len(ablate)):
            for p in eval('model.layer' + str(ablate[k]) + '.parameters()'):
                p.requires_grad = False
                ablated += p.numel()
        normal = sum(p.numel() for p in model.parameters() if p.requires_grad)
        if verbose_train: print(f'Tot param: {tot_params}  Ablated: {ablated}  Normal: {normal}    Pabl= {100*ablated/tot_params:.2f}%')
    ...
    history = {'update': [], 'train_loss': [fw0], 'val_loss': [initial_val_loss], 'train_acc': [train_accuracy],
               'val_acc': [test_accuracy], 'step_size': [0.05],
...
if __name__ == '__main__':
    ...
    inds_test = range(200)
    for abl in [1,2,3,4]:
        #trainset = Subset(torchvision.datasets.CIFAR10(root = dts_root, train = True, download = True, transform = transform), inds)
        trainset = torchvision.datasets.CIFAR10(root=dts_root, train=True, download=True, transform=transform)
        trainloader = torch.utils.data.DataLoader(trainset, batch_size = bs, shuffle = True)#, pin_memory = True,num_workers = nw)
