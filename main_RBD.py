...
def train_model(sm_root: str,
    ...
    for epoch in range(ep):
        ...
        for x, y in dts_train:
            ...
            optimizer.zero_grad()
            p = random.choice(list(model.parameters())) # For every batch we select randomly and update only one layer
            ...
            #p_after = p.data
            #epoch_update_list.append(torch.linalg.norm(p_after - p_before).item())
