...
def train_model(sm_root: str,
    ...
    # we select the blocks to update and the probability distribution of the random choices
    blocks = [[0, 0.25], [0.25, 0.7], [0.7, 1]]  # "percentage" of layers to update (from...to...)
    i = random.choices(blocks, weights = [0.05, 0.15, 0.8], k = ep) # for every epoch we select the block to be updated with probability = weights
    ...
    for epoch in range(ep):
        ...
        param_to_update_list = param_list[int(i[epoch][0] * L):int(i[epoch][1] * L)]  # new list of parameters to update --> req. grad = True
        for p in param_to_update_list:
            p.requires_grad = True
        #print("param. to update: - from layer", int(i[epoch][0] * L) + 1, "to layer", int(i[epoch][1] * L))
        ...
        for x, y in dts_train:
            ...
            batch_update_list = []
            for p in param_list:
                batch_update_list.append(p.data.clone())
            y_pred = model(x)
            ...
            for j in range(len(param_list)):
                batch_update_list[j] = torch.linalg.norm(batch_update_list[j] - param_list[j]).item()
