...
def train_model(sm_root: str,
    ...
    agg = [0.70, 1, 0.25, 0.7, 0.7, 1]  # Block division: "percentage" of layers to be updated (from...to...)
    i = 0

    param_to_update_list = param_list[
                           int(agg[i] * L):int(agg[i + 1] * L)]  # list of layers to update --> req. grad = True
    #print("param. to update: - from layer", int(agg[i] * L) + 1, "to layer", int(agg[i + 1] * L))
    ...
    for epoch in range(ep):
        ...
        if epoch + 1 in [10, 20]: # in which epochs we want to switch blocks
            # we deactivate the current block
            for p in param_to_update_list:
                p.requires_grad = False
            i += 2
            # we activate the new block
            param_to_update_list = param_list[int(agg[i] * L):int(agg[i + 1] * L)]  # new list of layers to update
            for p in param_to_update_list:
                p.requires_grad = True
            #print("param. to update: - from layer", int(agg[i] * L) + 1, "to layer", int(agg[i + 1] * L))
