...
class CMA_L_BD(torch.optim.Optimizer):
    ...
    def step(self, closure=None, *args, **kwargs):
        ...
        if closure is not None:
            with torch.no_grad():
                ...
                print("closure not none - compute loss:", loss)
        else:
            with torch.no_grad():
                for group in self.param_groups:
    ...
    def EDFL(self,
        ...
        if verbose: print(
            f'Starting EDFL  with alpha =  {alpha}    f_tilde = {f_tilde}    real_loss_before = {real_loss}')
        nfev += 1
        ...
        idx = 0
        f_j = f_tilde
        while cur_loss <= min(f_j,
                              real_loss - gamma * alpha * torch.linalg.norm(d_k) ** 2) and idx <= self.defaults.get(
                'max_it_EDFL'):
    ...
    def control_step(self,
        ...
        if f_tilde < min(fw0, phi - gamma * zeta):  # This is the best case, Exit at step 7
            ...
            if torch.linalg.norm(d) <= tau * zeta:  # Step 9, we check ||d||
                if verbose: print('||d|| suff small  -->  Step size reduced')
                ...
                # print(f'Real Loss computed with model_after_IC =  {real_loss}')
            ...
            if alpha > 0:  # If alpha is not zero, set the variables to the new value
                new_w = w_before + alpha * d
                set_w(model, new_w)
                # print('Loss with new_w at the end = ',closure(dl_train,model,criterion,device))

        if verbose: print(
            f'phi_before: {phi:3e} f_tilde: {f_tilde:3e} f_after: {f_after:3e}  Exit Step: {history["Exit"][-1]}')
